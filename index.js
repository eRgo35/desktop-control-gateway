...
const CONFIG = require('./config'); // Imoports settings
console.log(
  chalk.cyan(`[${NOW()}] LOADING > Config files loaded successfully!`)
);
...
try {
  ...
  console.log(
    chalk.green(`[${NOW()}] SUCCESS > SSL certificates loaded successfully!`)
  );
  SERVER = https.createServer(CERTS, app); // Tries to create a server
  SERVER.listen(PORT, () => {
    // Server starts to listen for incoming requests
    console.log(
      chalk.green(`[${NOW()}] SUCCESS > Listening at https://localhost:${PORT}`)
    );
  });
} catch (err) {
  // If SSL certs doesn't exist, tries too create an http server instead
  console.log(chalk.red(`[${NOW()}] WARNING > No SSL certificate found!`));
  if (CONFIG.FORCE_SSL !== true) {
    console.log(
      chalk.red(`[${NOW()}] WARNING > Starting using INSECURE http connection!`)
    );
    ...
    console.log(
      chalk.red(`[${NOW()}] WARNING > Server is shutting down`)
    );
    console.log(
      chalk.red(`[${NOW()}] WARNING > If you want to start server anyway`)
    );
    console.log(
      chalk.red(`[${NOW()}] WARNING > modify FORCE_SSL in CONFIG.JS file`)
    );
  }
}
...
app.post('/api/manage', (req, res) => {
  ...
  if (req.body.key === KEY) {
    ...
    switch (req.body.command) {
      case 'powerOn': // required arguments are: mac; optional: broadcast
        try {
          ...
          if (mac === undefined)
            throw { message: 'MAC address is not defined' };
      ...
      case 'powerOff': // required arguments are: ip address, username, password and shutdown command; optional: port
        try {
          ...
          if (username === undefined)
            throw { message: 'Username is not defined' };
          if (password === undefined)
            throw { message: 'Password is not defined' };
          if (cmd === undefined)
            throw { message: 'ShutdownCmd is not defined' };
    ...
    console.log(
      chalk.red(`[${NOW()}] WARNING > Unauthorized request detected`)
    );
  }
});
